<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:69:"D:\Apache24\htdocs\starcms./application/admin\view\index\content.html";i:1534687776;}*/ ?>
<!DOCTYPE html>
<html>

<head>

    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <!--360浏览器优先以webkit内核解析-->


    <title>雪狐ThinkPHP5内容管理系统后台首页</title>

    <link rel="shortcut icon" href="favicon.ico"> <link href="/public/static/admin/css/bootstrap.min14ed.css?v=3.3.6" rel="stylesheet">
    <link href="/public/static/admin/css/font-awesome.min93e3.css?v=4.4.0" rel="stylesheet">

    <link href="/public/static/admin/css/animate.min.css" rel="stylesheet">
    <link href="/public/static/admin/css/style.min862f.css?v=4.1.0" rel="stylesheet">
    

</head>

<body class="gray-bg">
    <div class="row  border-bottom white-bg dashboard-header">
        <div class="col-sm-12">
            <blockquote class="" style="font-size:16px">【StarCMS内容管理系统】非常感谢您的关注和支持…
                <h4 class="text-danger"> &nbsp;后台采用ThinkPHP5 + MySQL开发（H+ UI）</h4>
            </blockquote>

            <hr>
        </div>
        <div class="col-sm-3">
            <h2>作者微信</h2>
            <small>请扫描以下二维码：</small>
            <br>
            <br>
            <img src="/public/static/admin/img/weixin.jpg" width="120" style="max-width:264px;">
            <br>
        </div>
        <div class="col-sm-5">
            <h2>StarCMS简介</h2>
            <p style="line-height:2em">StarCMS是一个完全开源的CMS系统，程序采用最新的THINKPHP5.0和MYSQL数据库，后台UI基于Bootstrap3.3.6最新版本开发的扁平化主题，使用了Html5+CSS3等现代技术，整合了Jstree多级菜单，Ueditor编辑器，Ajaxuploadfile图片上传，Webuploader文件上传等组件,系统集成了最新的阿里云短信，PHPMailer邮件发送，支付宝，微信支付，七牛云存储等接口，可以用于所有的Web应用程序，如<b>企业展示型网站</b>，<b>信息发布网站</b>，<b>电商网站</b>，<b>视频网站</b>，<b>资源下载网站</b>，<b>教育政府协会类网站</b>等等，当然，您也可以对她进行深度定制，以做出更强系统。</p>
            <p>
                <b>当前版本：</b>v1.0.0
            </p>
           
            <br>

        </div>
        <div class="col-sm-4">
            <h4>最近更新：</h4>
            <ol>
                <li>完全响应式布局（支持电脑、平板、手机等所有主流设备）</li>
                <li>基于最新版本的Bootstrap 3.3.6</li>
                <li>提供3套不同风格的皮肤</li>
                <li>支持多种布局方式</li>
                <li>使用最流行的的扁平化设计</li>
                <li>提供了诸多的UI组件</li>
                <li>集成多款国内优秀插件，诚意奉献</li>
                <li>提供盒型、全宽、响应式视图模式</li>
                <li>采用HTML5 & CSS3</li>
                <li>拥有良好的代码结构</li>
                <li>更多……</li>
            </ol>
        </div>

    </div>
    <div class="wrapper wrapper-content">
        <div class="row">
            <div class="col-sm-4">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5>联系信息</h5>

                    </div>
                    <div class="ibox-content">
                        <p><i class="fa fa-send-o"></i> 网站：<a href="http://www.txxwl.net/" target="_blank">http://www.txxwl.net</a>
                        </p>
                        <p><i class="fa fa-qq"></i> QQ：<a href="http://wpa.qq.com/msgrd?v=3&amp;uin=1045746362&amp;site=qq&amp;menu=yes" target="_blank">1045746362</a>
                        </p>
                        <p><i class="fa fa-weixin"></i> 微信：<a href="javascript:;">15822422406</a>
                        </p>
                        <p><i class="fa fa-credit-card"></i> 支付宝：<a href="javascript:;" class="支付宝信息">zxh5566@tom.com / 张*华</a>
                        </p>
                    </div>
                </div>
            </div>
            <div class="col-sm-4">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5>系统特点</h5>
                    </div>
                    <div class="ibox-content">
					<ol>
                            <li>架构轻盈，完全免费与开源；</li>
                            <li>让简洁与强大并存；</li>
                            <li>顶级搜索引擎优化功能；</li>
                            <li>会员、留言、投稿、支付购物；</li>
                            <li>超级精美的模版</li>
                        </ol>
                    </div>
                </div>
            </div>
            <div class="col-sm-4">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5>系统功能</h5>
                    </div>
                    <div class="ibox-content">
                        <ol>
                            <li>模型(强大自定义功能)，可以自由添加模型和模型字段；</li>
                            <li>支付管理、购物管理、会员管理、广告管理、链接管理等；</li>
                            <li>静态HTML生成、自定义模板标签；</li>
                            <li>独立的页面优化信息设置，方便快捷；</li>
                            <li>全站纯静态生成功能</li>
                            <li>分组机制管理权限设定，全功能全栏目均可指定管理权限</li>
                            <li>全站高效缓存机制，运行飞速体验</li>
                        </ol>
                       

                    </div>
                </div>
            </div>
        </div>
    </div>
    <script id="welcome-template" type="text/x-handlebars-template">
        <div class="border-bottom white-bg page-heading clearfix">
            <h2>更新日志：</h2>
            <div>今天是情人节，H+终于跨到了v3.0，就算是情人节礼物吧，感谢你们的不离不弃，一路相伴！</div>
            <div class="pull-right">——StudyFox / 2015.8.20</div>
        </div>
        <div class="m">
            <div class="tabs-container">
                <div class="tabs-left">
                    <ul class="nav nav-tabs">
                        <li class="active">
                            <a data-toggle="tab" href="#layouts"><i class="fa fa-columns"></i> 布局
                            </a>
                        </li>
                        <li>
                            <a data-toggle="tab" href="#new"><i class="fa fa-plus-square"></i> 新增
                            </a>
                        </li>
                        <li>
                            <a data-toggle="tab" href="#update"><i class="fa fa-arrow-circle-o-up"></i> 升级
                            </a>
                        </li>
                        <li>
                            <a data-toggle="tab" href="#revise"><i class="fa fa-pencil"></i> 修正
                            </a>
                        </li>
                        <li>
                            <a data-toggle="tab" href="#optimize"><i class="fa fa-magic"></i> 优化
                            </a>
                        </li>
                    </ul>
                    <div class="tab-content" style="line-height:1.8em;">
                        <div id="layouts" class="tab-pane active">
                            <div class="panel-body">
                                <ol class="no-left-padding">
                                    <li class="text-danger"><b>推荐：</b>期待已久的contentTabs效果，支持关闭、双击刷新、左右滑动等；</li>
                                    <li>固定左侧主菜单栏，并对菜单项做了新的调整；</li>
                                    <li>增加右侧面板及聊天窗口等。</li>
                                </ol>

                                <p style="margin-left:25px;">
                                    <hr><span class="label label-danger">特别致谢</span> 感谢[子·梦]同学提供的contentTabs优化方案和代码！</p>
                            </div>
                        </div>
                        <div id="new" class="tab-pane">
                            <div class="panel-body">
                                <ol class="no-left-padding">
                                    <li>表单：搜索自动补全插件suggest、高级表单插件（时间选择，切换按钮，图像裁剪上传，单选复选框美化，文件域美化等)等；</li>
                                    <li>图表：图表组合页面等；</li>
                                    <li>页面：团队、社交、客户管理、文章列表、文章详情、新登录页面等；</li>
                                    <li>UI元素：竖向选项卡、拖动面板、文本对比、加载动画、SweetAlert等；</li>
                                    <li>相册：layer相册、Blueimp相册等；</li>
                                    <li>表格：FooTables等。</li>
                                </ol>
                            </div>
                        </div>
                        <div id="update" class="tab-pane">
                            <div class="panel-body">
                                <ol>
                                    <li>页面弹层插件layer升级至1.9.3；</li>
                                    <li>更新jqgrid，支持树形表格；</li>
                                    <li>更新帮助文档。</li>
                                </ol>
                            </div>
                        </div>
                        <div id="revise" class="tab-pane">
                            <div class="panel-body">
                                <ol>
                                    <li>jstree、Simditor等多处错误；</li>
                                    <li>页面加载进度提示；</li>
                                    <li>Glyphicon字体图标不显示的问题；</li>
                                    <li>重新整理开发文档；</li>
                                </ol>
                            </div>
                        </div>
                        <div id="optimize" class="tab-pane">
                            <div class="panel-body">
                                <ol>
                                    <li>H+整体视觉效果；</li>
                                    <li>jstree默认主题显示效果；</li>
                                    <li>表单验证显示效果；</li>
                                    <li>iCheck显示效果；</li>
                                    <li>Tabs显示效果。</li>
                                </ol>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="alert alert-warning alert-dismissable m-t-sm">
                <button aria-hidden="true" data-dismiss="alert" class="close" type="button">×</button>
                同时这也是一个示例，演示了如何从iframe中弹出一个覆盖父页面的层。
            </div>
        </div>
    </script>
    <script src="/public/static/admin/js/jquery.min.js?v=2.1.4"></script>
    <script src="/public/static/admin/js/bootstrap.min.js?v=3.3.6"></script>
    <script src="/public/static/admin/js/plugins/layer/layer.min.js"></script>
    <script src="/public/static/admin/js/content.min.js"></script>
    <script src="/public/static/admin/js/welcome.min.js"></script>
</body>

</html>
